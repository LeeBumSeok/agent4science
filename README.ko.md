...
ChatGPT Pro는 OpenAI의 가장 강력한 추론 모드를 제공합니다 — 연구에서 *어려운* 부분,
즉 아이디어가 참신한지 판단하고, 실험 설계의 허점을 찌르고, 대안을 저울질하고, 지저분한
결과를 해석하는 데 현재 가장 뛰어납니다. 하지만 그 능력은 **웹 앱 안에** 있습니다.
터미널에서 조작할 수 없고, CLI 에이전트 루프에 현실적으로 엮을 수도 없습니다 — 전용 CLI가
없고, 최상위 티어를 스크립트로 돌리는 것도 저렴하지 않으니까요. 반대로 CLI에서 돌릴 수 있는
모델들은 구현은 훌륭하지만 그런 깊은 연구 판단에는 약합니다.

oh-my-ai4science는 이 간극을 잇습니다: **어려운 사고는 ChatGPT Pro에서 하고, 그 대화 전체를
OpenCode로 넘기면** CLI 에이전트가 실험을 구현·실행·분석하고, 결과를 다시 Pro로 가져갈
프롬프트까지 만들어 다음 반복으로 이어줍니다. 똑똑한 연구 판단은 웹에, 재현 가능한 실행은
당신의 터미널에 둡니다.
